...
设计稿一律由 **`design.html` 渲染成 `design.png`**，而不是用生图模型产出 —— ground truth 精确可控，任何人都能逐像素复现。
...
**注入纪律。** 每处缺陷都经 `assert.mjs` 三层校验（字符串锚点唯一 / 声明区块内像素确有变化 / 变化不越界到未声明区块）。第二层专门用来抓「改了 CSS 但视觉无变化」的假注入，例如：

> 当前字体栈下 `font-weight` 600/700/800/900 渲染为同一字面，把主标题 800→600 的注入在像素上完全无效。
