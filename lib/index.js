...
function getRemote(rawUrl, hops = 0) {
	return new Promise((resolveWith, rejectWith) => {
		...
		const headers = {
			host: target.host,
			accept: "image/*,*/*;q=0.8",
			"user-agent": "dsh-deepseek-vision"
		};
		...
		const onError = (error) => {
			...
			fail(/* @__PURE__ */ new Error(`dsh-deepseek-vision: 连不上 ${target.origin}（${code}）` + (proxy === void 0 ? "。" : `，用的代理是 ${proxy.origin}（经代理时域名拼错也会报成连接被重置，先核对地址）。`) + " 网络不通或该站点需要登录态时，最省事的办法是**先把图下载到本地，再把本地路径传给本工具**，例如 `curl -L -o /tmp/x.png <地址>`。 不要改用读源码、猜内容或自己做像素分析来代替看图 —— 那既慢又不可靠。"));
		};
		const arm = (request$2) => {
			open.push(request$2);
		...
		tunnel.on("connect", (proxyResponse, socket) => {
			...
			if (proxyResponse.statusCode !== 200) {
				socket.destroy();
				fail(/* @__PURE__ */ new Error(`dsh-deepseek-vision: 代理 ${proxy.origin} 拒绝了 CONNECT（HTTP ${proxyResponse.statusCode}）。请检查代理设置或改用本地文件路径。`));
				return;
			}
			arm(request$1({
				socket,
...
async function fetchRemoteImage(url) {
	...
	const response = await getRemote(url).catch((error) => {
		if (error instanceof TooLargeError) throw new Error(`dsh-deepseek-vision: 图片 ${mb(error.bytes)}，超过上限 ${mb(MAX_REMOTE_BYTES)}。 识图按像素计费，超大图既贵又会被降采样。请先裁剪或压缩到上限以内再传，或只截取你真正要看的那一块 —— 局部图的判定精度本来也更高。`);
		throw error;
	});
	if (response.status < 200 || response.status >= 300) throw new Error(`dsh-deepseek-vision: 下载图片失败 HTTP ${response.status}。 若该图需要登录态（飞书、Notion 等私有文档），请先用对应的 skill 或带凭据的命令把它下载到本地，再把本地路径传给本工具。`);
