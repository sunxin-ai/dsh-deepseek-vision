 ...
 * | **「各自是多少」**      | 0/12 | **2/2** |
 *
 * 上表四种问法里，评测脚本只实现了「这个方面有区别吗」那一种（`eval/tilebench.py` 的 `FACETS`），
 * 它的 0/30 有逐格原文可查；另外两行没有代码路径，数字是手工实测记录，别当可复现基准引用。
 * 三条伪影的细节见 `skills/design-qa/SKILL.md`，复核范围见 `eval/README.md`。
 *
